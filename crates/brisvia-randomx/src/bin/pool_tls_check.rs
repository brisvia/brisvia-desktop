...
fn main() {
    ...
    match c.login(&addr, "tls-check", Duration::from_secs(20)) {
        ...
        Ok(None) => {
            ...
            // A confirmed login is not enough: the work has to arrive afterward, or the miner stays
            // connected without mining. We wait for the first job just as the real session does.
        ...
        Err(LoginError::Permanent(m)) => {
            println!("   OK: the pool answered and REJECTED explicitly -> {m}");
            println!("      (expected if the address does not belong to this pool's network)");
        }
