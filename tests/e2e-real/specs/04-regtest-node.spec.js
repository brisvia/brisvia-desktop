// P0 flow #4 — Real regtest node: the app brings up the node and shows network and height.
// On the real COMPILED app (Rust backend + ephemeral regtest bitcoind), verifies that:
//   - the backend actually brings up the node: the RPC responds (waitRpcUp);
...
describe('Flow 4 — regtest node', () => {
  it('brings up the node, the RPC responds and the app shows network and height', async () => {
    ...
    }, { timeout: 20000, timeoutMsg: `the displayed height did not match the node's (${nodeHeight})` });
  });
});
