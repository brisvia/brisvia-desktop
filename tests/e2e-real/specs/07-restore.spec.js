// P0 flow #7 — Restore wallet from 12 words (real "switch PC" flow).
// On the real COMPILED app (real Rust backend, without depending on the node), verifies that:
//   - onboarding lets you choose "import" and shows the grid of 12 fields;
//   - you can type 12 valid words (standard BIP39 phrase) and move on;
//   - a new password is requested to encrypt the restored wallet;
//   - the backend actually restores (wallet.restore) and leaves onboarding showing the wallet with an address.
// This is "real backend without node": restoring does not depend on the node being up.
'use strict';
...
describe('Flow 7 — restore wallet', () => {
  it('imports 12 valid words, asks for a password and enters the wallet with an address', async () => {
    ...
    await browser.waitUntil(async () => (await importGrid.$$('input')).length === 12, {
      timeout: 10000, timeoutMsg: 'the import grid did not show 12 fields',
    });

    // 2) Type the 12 words (one per field, in order).
