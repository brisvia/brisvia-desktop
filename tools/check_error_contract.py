...
Spanish UI. An earlier review of the logic never looked at this, because each half is fine on
...
def leer(p):
    if not p.exists():
        fallos.append(f"file not found {p}")
        return ""
...
for e in sorted(rust_errs):
    if e not in mapa:
        fallos.append(
            f"Rust can return ERR:{e} but the program does not know how to show it: "
            f"'{e}' is missing in transError (app.js). The user would see raw text."
        )
...
for e in sorted(mapa):
    if e not in rust_errs:
        fallos.append(
            ...
            f"It is extra, or someone renamed the error and left dead text behind."
        )
