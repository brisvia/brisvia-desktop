...
print(f"keys ES: {len(kes)} | keys EN: {len(ken)}")
...
for m in re.finditer(r'<(input|textarea)[^>]*placeholder="([^"]+)"[^>]*>', html):
    if "data-i18n" not in m.group(0) and m.group(2) not in PERMITIDO:
        fallos.append(
            f'hard-coded text in the HTML that is never translated: placeholder "{m.group(2)}" '
...
for m in re.finditer(
    ...
    if not txt or re.fullmatch(r"[\W\d\s·—…]+", txt) or txt.startswith("&") or txt in PERMITIDO:
        continue
    fallos.append(f'hard-coded text in the HTML that is never translated: <{m.group(1)}>"{txt[:50]}"')
...
if fallos:
    ...
    for f in fallos:
        print("  - " + f)
    sys.exit(1)
print("OK: both languages are complete, with no mixing, and the HTML has no hard-coded untranslated text.")
