...
try:
    ...
    print("Nobody learns about the update. latest.json needs to be uploaded to the release marked as 'latest'")
...
for falta in ESPERADAS - set(plats):
    fallos.append(f"platform {falta} is missing: those people get no update and never find out")

for plat, v in plats.items():
    if not v.get("signature", "").strip():
        fallos.append(f"{plat}: empty signature (the app rejects an update without a valid signature)")
    url = v.get("url", "")
    code = head(url)
