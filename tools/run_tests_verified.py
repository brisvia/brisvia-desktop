...
def main():
    ...
    if corridos == 0:
        fallos.append(
            "IT RAN ZERO TESTS. The command may have 'passed' without testing anything: a filter that matches nothing, "
            "a renamed module, a feature that hides them. Green without having tested anything is worse than red."
        )
    elif corridos < a.min:
        fallos.append(
            f"it ran {corridos} tests but at least {a.min} were expected. Either tests were lost, or a suite "
    ...
    if a.min_seconds and dur < a.min_seconds and not fallos:
        fallos.append(f"it finished in {dur:.1f}s, suspiciously fast (>{a.min_seconds}s were expected)")
