...
function Test-PowerShellParses {
    ...
    $limpio = [regex]::Replace($Code, '\$\{\{[^}]*\}\}', "'gha_expression'")
    $errores = $null
    [void][System.Management.Automation.Language.Parser]::ParseInput($limpio, [ref]$null, [ref]$errores)
    if ($errores -and $errores.Count -gt 0) {
        return ($errores[0].Message -split "`n")[0]
    }
    return $null
}
...
function Get-InlinePowerShellBlock {
    ...
    for ($i = 0; $i -lt $Lines.Count; $i++) {
        if ($Lines[$i] -match '^\s*-?\s*name:\s*(.+?)\s*$') { $nombre = $Matches[1].Trim("'", '"') }
        ...
        for ($k = $i + 1; $k -lt $Lines.Count; $k++) {
            $l = $Lines[$k]
            if ($l.Trim() -eq '') { $cuerpo += ''; continue }
            $actual = $l.Length - $l.TrimStart().Length
            if ($actual -le $sangria) { break }
            $cuerpo += $l
        }
        if ($shell -match 'pwsh|powershell') {
            $bloques += [pscustomobject]@{
                Name = $nombre; Line = $i + 1; Code = ($cuerpo -join "`n")
            }
        }
    }
    return $bloques
...
if ($SelfTest) {
    ...
    if ($null -eq $r) { Write-Host '  FAIL  metatest: the real bug parses clean; the check is blind'; $fallos++ }
    else { Write-Host "  PASS  metatest-catches-the-real-bug  ($r)" }

    $arreglado = 'function T($path, $filter) { throw "FAIL: copies of ${filter}: fine" }'
    if ($null -ne (Test-PowerShellParses -Code $arreglado)) {
        Write-Host '  FAIL  metatest: the fix does not parse'; $fallos++
    } else { Write-Host '  PASS  metatest-the-fix-parses' }

    if ($null -ne (Test-PowerShellParses -Code 'if (${{ matrix.os }} -eq 1) { "x" }')) {
        Write-Host '  FAIL  metatest: a GitHub expression is reported as a syntax error'; $fallos++
    ...
    if ($b.Count -ne 1) { Write-Host "  FAIL  metatest: found $($b.Count) pwsh blocks, expected 1"; $fallos++ }
    elseif ($b[0].Name -ne 'a step') { Write-Host "  FAIL  metatest: named it '$($b[0].Name)'"; $fallos++ }
    ...
    if ($b2.Count -ne 2) {
        Write-Host "  FAIL  metatest: found $($b2.Count) blocks, expected 2"; $fallos++
    } elseif ($b2[0].Code -match '- name:') {
        Write-Host '  FAIL  metatest: the body reader swallowed the next step'; $fallos++
    } elseif ($null -ne (Test-PowerShellParses -Code $b2[0].Code)) {
        Write-Host '  FAIL  metatest: a clean one-line block is reported as a parse error'; $fallos++
    } else { Write-Host '  PASS  metatest-body-reader-stops-at-the-next-step' }

    Write-Host ''
    if ($fallos) { Write-Host "SELF-TEST FAILED ($fallos)"; exit 1 }
...
Write-Host "  $($bloques.Count) inline PowerShell blocks"
Write-Host ''
...
if ($malos) {
    Write-Host ''
    Write-Host "FAIL: $malos of $($bloques.Count) blocks do not parse. They would die before their first line."
    exit 1
}
Write-Host "  all $($bloques.Count) parse."
exit 0
