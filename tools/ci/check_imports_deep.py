...
WHY
---
The claim "no symbol is missing" from the rc6 test executable does not hold up. To be complete, "all
symbols exist" has to be widened to: every name, every ordinal, and every final target of forwarded
exports. Until all three are covered, the contradiction is not complete.

The original comparison took names out of dumpbin's text output. An import by ORDINAL has no name to
compare, so a name-only diff reports it as absent rather than as unmatched -- it disappears instead of
failing. And an export can be a FORWARDER: the name is present in the first DLL and merely points
somewhere else, which a name check reads as satisfied.

There is a precedent: windows-rs hit exactly this 0xC0000139 because GetWindowSubclass got linked
...
def self_test():
    ...
    for cand in (r"C:\Windows\System32\notepad.exe", r"C:\Windows\System32\cmd.exe"):
        ...
        if not imp or total < 5:
            print(f"  FAIL  {p.name}: {len(imp)} dlls / {total} symbols -- too few to be real")
            fallos += 1
        else:
            print(f"  PASS  reads-a-real-PE  ({p.name}: {len(imp)} dlls, {total} symbols)")
        break
    else:
        print("  FAIL  found no Windows binary to test against")
        fallos += 1
...
def main():
    ...
    if a.self_test:
        return self_test()
    if not a.exe:
        return ap.error("an .exe is required")
    ...
    print()
    if por_ordinal:
        print("  IMPORTS BY ORDINAL FOUND. A name-only diff does NOT see them:")
        for dll, ords in por_ordinal:
            print(f"    {dll}: {ords}")
        print("  Each one must be resolved against the /EXPORTS of the DLL that Windows actually loads.")
    else:
        print("  Zero imports by ordinal. That route is ruled out by reading the PE directly,")
        print("  not by never having looked at it.")
    return 0
