...
DEST = REPO / ".github/workflows/validate-windows-fix.yml"

# Copy up to and including the app build. Everything past it publishes.
LAST_COPIED_STEP = "npm run tauri build"

HEADER = """# GENERATED by tools/ci/derive_validation_workflow.py -- DO NOT EDIT BY HAND.
#
...
def build_steps(text):
    ...
    if cut < 0:
        raise SystemExit(f"could not find '{LAST_COPIED_STEP}' -- the release workflow changed shape")
    end = text.find("\n      - name:", cut)
    if end < 0:
        end = len(text)
    ...
    block, n = signed_step.subn(validation_step, block)
    if n != 1:
        raise SystemExit(
...
def generate():
    return HEADER + build_steps(SOURCE.read_text(encoding="utf-8")) + "\n" + VALIDATION
...
def main():
    ...
    if a.self_test:
        ...
        failures = 0
        b = build_steps(t)
        for needed in ("randomx", "vcpkg", "bitcoind", "dtolnay/rust-toolchain"):
            if needed not in b.lower():
                print(f"  FAIL  the copied block has no {needed}: it would not build")
                failures += 1
            else:
                print(f"  PASS  copies-{needed}")
        for forbidden in ("softprops/action-gh-release", "latest.json", "minisign"):
            if forbidden in b:
                print(f"  FAIL  the copied block contains {forbidden}: it could publish")
                failures += 1
        ...
        if "\t" in generate():
            ...
            print("  PASS  no-tabs")
        print()
        print("SELF-TEST FAILED" if failures else "self-test OK: the derived block builds and cannot publish")
    ...
    if a.check:
        ...
        if old_text != new_text:
            print(f"FAIL: {DEST.name} has drifted from build-windows.yml.")
    ...
    print(f"  {len(new_text.splitlines())} lines, build steps copied from build-windows.yml")
    return 0
