...
Why it exists: the frontend (app.js transError) shows AS-IS any string it does not recognize. If the
...
def main() -> int:
    ...
    #    "ERR:CODE" appears in the comment explaining the format: it is not a real code.
    codigos = {c[4:] for c in re.findall(r'"(ERR:[A-Z_]+)"', rust)} - {"CODE"}
    # The map packs several per line; without a start-of-line anchor (a ^ here would only see the first).
    ...
    for k in sorted(claves):
        ...
        if n == 0:
            ...
            fallos.append(f"errors.{k} has text in {n} language(s): translations are missing")
    ...
    if cuerpo and re.search(r"^\s*msg\.to_string\(\)\s*$", cuerpo.group(0), re.M):
        fallos.append(
            "friendly_error returns msg.to_string() as fallback: the node's raw text (with personal paths "
            "and the word 'Bitcoin') would reach the user. It must return a sanitized code."
        )
    ...
    if fallos:
        ...
        for f in fallos:
            print(f"  - {f}")
        return 1
    print("OK: every backend error reaches the user translated, in both languages, and none raw.")
    return 0
